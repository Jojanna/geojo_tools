...
def reflectivity_2layer(ai1, ai2):
    r = (ai2 - ai1) / (ai2 + ai1)
    return r

def shuey_2term_2layer(vp1, vp2, vs1, vs2, rho1, rho2):
    vp_mean = ((vp2+vp1)/2)
    vs_mean = ((vs2+vs1)/2)
...
def shuey_3term_2layer (vp1, vp2, vs1, vs2, rho1, rho2):
    vp_mean = ((vp2+vp1)/2)
    vs_mean = ((vs2+vs1)/2)
    rho_mean = ((rho2+rho1)/2)
    g = 0.5 * (vp2 - vp1)/vp_mean - 2 * pow(vs_mean,2)/pow(vp_mean,2) * ((rho2-rho1)/rho_mean + 2 * (vs2-vs1)/vs_mean)
...
def shuey_3term_log (vp, vs, rho, depth):
...
def ref_3term_theta (intercept, gradient, f, theta):

    rad = math.radians(theta)
    r = intercept + gradient * np.power(np.sin(rad), 2) +  f * (np.power(np.tan(rad), 2) - np.power(np.sin(rad), 2))

    return r


def elastic_impedance_log(vp, vs, rho, theta):
